...
<div class="max-w-3xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="relative rounded-2xl overflow-hidden
        ...
        <div class="relative p-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-extrabold text-white tracking-tight">Edit jadwal pelajaran</h1>
                <p class="text-blue-200 dark:text-white/40 text-sm mt-0.5">Perbarui jadwal {{ $jadwal->mapel->nama_mapel }}</p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.jadwal.update', $jadwal->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="guru_id" required class="{{ $ic }} @error('guru_id') border-red-400 @enderror">
                        ...
                        @foreach($gurus as $g)
                            <option value="{{ $g->id }}" {{ $jadwal->guru_id == $g->id ? 'selected' : '' }}>{{ $g->nama }} ({{ $g->nip }})</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <input type="time" name="jam_mulai" value="{{ $jadwal->jam_mulai }}" required class="{{ $ic }} @error('jam_mulai') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="time" name="jam_selesai" value="{{ $jadwal->jam_selesai }}" required class="{{ $ic }} @error('jam_selesai') border-red-400 @enderror">
            ...
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-6 pt-5 border-t border-gray-100 dark:border-white/[0.07]">
                ...
                <button type="submit" class="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-bold text-sm transition flex items-center justify-center gap-2">
                    <i data-lucide="save" class="w-4 h-4"></i> Update jadwal
                </button>
            </div>
        </form>
    </div>
