...
<div class="max-w-3xl mx-auto px-4 py-6 space-y-6">
    ...
    <div class="relative rounded-2xl overflow-hidden
        ...
        <div class="relative p-6 flex items-center gap-3">
            ...
            <div>
                <h1 class="text-xl font-extrabold text-white tracking-tight">Edit mata pelajaran</h1>
                <p class="text-blue-200 dark:text-white/40 text-sm mt-0.5">Perbarui data {{ $mapel->nama_mapel }}</p>
            </div>
        </div>
    </div>
        ...
        <form action="{{ route('admin.mapel.update', $mapel->id) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div>
                    ...
                    <input type="text" name="kode" value="{{ old('kode', $mapel->kode) }}" required placeholder="Contoh: MAT" class="{{ $ic }} @error('kode') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="text" name="nama_mapel" value="{{ old('nama_mapel', $mapel->nama_mapel) }}" required placeholder="Contoh: Matematika" class="{{ $ic }} @error('nama_mapel') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <input type="number" name="kkm" value="{{ old('kkm', $mapel->kkm ?? 75) }}" min="0" max="100" required placeholder="75" class="{{ $ic }} @error('kkm') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <select name="kategori" required class="{{ $ic }} @error('kategori') border-red-400 @enderror">
                        ...
                        <option value="peminatan" {{ old('kategori', $mapel->kategori) == 'peminatan' ? 'selected' : '' }}>Peminatan</option>
                    </select>
            ...
            <div class="flex flex-col-reverse sm:flex-row sm:justify-end gap-3 mt-6 pt-5 border-t border-gray-100 dark:border-white/[0.07]">
                ...
                <button type="submit" class="px-5 py-2.5 bg-blue-700 hover:bg-blue-800 text-white rounded-xl font-bold text-sm transition flex items-center justify-center gap-2">
                    <i data-lucide="save" class="w-4 h-4"></i> Update mapel
                </button>
            </div>
        </form>
    </div>
