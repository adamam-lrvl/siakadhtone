        ...
        <div class="relative p-6">
            <div class="flex items-start justify-between gap-4 mb-4">
                <div class="min-w-0">
                    ...
                    <p class="text-blue-200 dark:text-white/40 text-sm mt-1">Kelola kelas dan wali kelas sekolah</p>
                </div>
            ...
            <form action="{{ route('admin.kelas.index') }}" method="GET" class="flex gap-2">
                <div class="flex-1 relative">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}"
                           autocomplete="off" placeholder="Cari nama kelas atau wali kelas..."
                ...
                <button type="submit"
                        class="px-4 py-2.5 bg-white/15 hover:bg-white/25 border border-white/20
                               text-white text-sm font-semibold rounded-xl transition flex-shrink-0">Cari</button>
                @if(request('search'))
    ...
    <div class="bg-white dark:bg-white/[0.05] dark:backdrop-blur-xl rounded-2xl border border-gray-200 dark:border-white/[0.07] overflow-hidden">
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/[0.05]">
                    @forelse($kelas as $k)
                        <tr class="hover:bg-indigo-50/30 dark:hover:bg-white/[0.04] transition
                            ...
                            <td class="px-5 py-4 font-semibold text-gray-900 dark:text-white/90">{{ $k->nama_kelas }}</td>
                            <td class="px-5 py-4">
                                @if($k->waliKelas)
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="font-medium text-gray-700 dark:text-white/70">{{ $k->waliKelas->nama }}</span>
                                    </div>
                                @else
                            ...
                            <td class="px-5 py-4 text-center">
                                <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold
                                    {{ $k->siswas_count > 0
                                        ...
                                        : 'bg-gray-100 dark:bg-white/[0.06] text-gray-500 dark:text-white/40 border border-gray-200 dark:border-white/[0.08]' }}">
                            ...
                            <td class="px-5 py-4">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <form action="{{ route('admin.kelas.destroy', $k->id) }}"
                                          method="POST" class="inline delete-form" data-name="{{ $k->nama_kelas }}">
                                        @csrf @method('DELETE')
                                        <button type="submit"
        ...
        <div class="sm:hidden divide-y divide-gray-100 dark:divide-white/[0.05]">
            ...
            <div class="p-4 hover:bg-gray-50 dark:hover:bg-white/[0.04] transition">
                ...
                <div class="pt-2 border-t border-gray-100 dark:border-white/[0.05] mt-2">
                    @if($k->waliKelas)
                        <div class="flex items-center gap-2">
                            <i data-lucide="user" class="w-3.5 h-3.5 text-gray-400 dark:text-white/30 flex-shrink-0"></i>
                            <span class="text-xs text-gray-600 dark:text-white/60 font-medium">{{ $k->waliKelas->nama }}</span>
                        </div>
                    @else
                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold
                                     bg-red-50 dark:bg-red-500/10 text-red-600 dark:text-red-400
            ...
            @empty
                <div class="text-center py-16">
                    ...
                    <p class="text-sm font-medium text-gray-400 dark:text-white/30">Belum ada data kelas</p>
                </div>
            @endforelse
        </div>
